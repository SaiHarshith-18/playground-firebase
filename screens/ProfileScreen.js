...
export default function ProfileScreen() {
      ...
      const fetchProfile = async () => {
        ...
        try {
          ...
          if (userSnap.exists()) {
            ...
            setProfileData(data);
            setFormData({ fullName: data.fullName || '', about: data.about || '' });
  ...
  const handleSave = async () => {
    ...
    try {
      const userRef = doc(db, 'users', user.uid);
      const updatedProfile = { ...profileData, fullName: formData.fullName, about: formData.about };
...
const styles = StyleSheet.create({
  ...
  addPhotoIcon: { position: 'absolute', bottom: 0, right: 0, backgroundColor: '#FF822B', borderRadius: 14, width: 28, height: 28, alignItems: 'center', justifyContent: 'center', borderColor: '#fff', borderWidth: 2 },
