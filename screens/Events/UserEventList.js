...
export default function UserEventList() {
  ...
  const filterEvents = data => {
    ...
    return data.filter(event => {
      ...
      if (filters.gameType && event.gametype?.toLowerCase() !== filters.gameType.toLowerCase())
        return false;
        ...
        <View style={styles.iconBar}>
          ...
          >
            {[
              ...
              <Menu.Item
                ...
                titleStyle={
                  sortBy === key ? { color: '#fff', fontWeight: 'bold' } : { fontWeight: 'normal' }
                }
                right={() =>
                  sortBy === key ? <Ionicons name="checkmark" size={18} color="#fff" /> : null
                }
              />
            ))}
          </Menu>
          ...
          >
            ...
            <List.Item
              ...
              style={filters.challenging ? { backgroundColor: '#FF822B' } : null}
              titleStyle={filters.challenging ? { color: '#FFF', fontWeight: 'bold' } : null}
            />
            {/* Time Range Filters */}
            {['today', 'week', 'month'].map(range => (
              <List.Item
                ...
                titleStyle={
                  filters.timeRange === range ? { color: '#FFF', fontWeight: 'bold' } : null
                }
              />
            ))}
            ...
            {['upcoming', 'past'].map(status => (
              <List.Item
                ...
                titleStyle={
                  filters.status === status ? { color: '#FFF', fontWeight: 'bold' } : null
                }
              />
            ))}
            <List.Item
