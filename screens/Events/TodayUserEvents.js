...
export default function TodayUserEvents({ navigation }) {
        ...
        const today = snapshot.docs
          ...
          .filter(event => {
            ...
            return (
              ...
              (event.createdBy === user.uid || (event.attendees || []).includes(user.uid))
            );
          });
  ...
  if (loading) return <ActivityIndicator size="small" style={{ marginTop: 10 }} />;
  ...
  return (
    <View style={styles.eventsContainer}>
      ...
      >
        <Ionicons name="football-outline" size={30} color="#FF822B" style={{ marginRight: 6 }} />
        <Text style={styles.createEventLink}>Create Event</Text>
      </TouchableOpacity>
    </View>
  );
